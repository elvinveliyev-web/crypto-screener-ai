...
# --- BORSA MOTORU (KuCoin - ABD Erişimi Uyumlu) ---
exchange = ccxt.kucoin({'enableRateLimit': True})

# --- TEKNİK ANALİZ MOTORU (MASTER 5 LOGIC) ---
def calculate_master_indicators(df):
    """Borsa uygulamasındaki tüm profesyonel indikatörleri hesaplar."""
    if df.empty or len(df) < 50:
        return df
    
    # Hareketli Ortalamalar (Master 5 Standartları)
    ...
    # Özel Formasyon: Kanguru Kuyruğu (Kangaroo Tail)
...
@st.cache_data(ttl=120) # 2 dakikalık hızlı önbellek
...
def get_top_symbols():
    try:
        ...
        # Sadece USDT paritelerini ve hacimli olanları getir
        symbols = [s for s, t in tickers.items() if s.endswith('/USDT') and ':' not in s]
        return sorted(symbols)
    except:
        return ["BTC/USDT", "ETH/USDT", "SOL/USDT"]

# --- YAN MENÜ (SIDEBAR) - Bire bir Borsa Uygulaması Düzeni ---
...
# SEKME 1: Bire bir Borsa Uygulaması Analiz Ekranı
with tab1:
    with st.spinner(f"{selected_symbol} Analiz Ediliyor..."):
        ...
        if not df.empty:
            ...
            if use_ta:
                # Master 5 Hareketli Ortalamalar
                ...
                # Bollinger Bantları (Opsiyonel görselleştirme)
            ...
            # Temel Veri Paneli (Hisse uygulamasındaki gibi)
            if use_fa:
                st.markdown("---")
...
# SEKME 2: Çoklu Tarayıcı (Screener) - İstek Üzerine Çalışan Versiyon
with tab2:
    ...
    if st.button("🚀 Taramayı Başlat"):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_coin)):
                ...
                time.sleep(0.05) # Rate limit freni
